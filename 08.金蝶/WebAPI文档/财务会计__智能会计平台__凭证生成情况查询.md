# 凭证生成情况查询 Web API 操作原始内容

> 来源页面：全部 > 财务会计 > 智能会计平台 > 凭证生成情况查询

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 3326 |
...
client.GetSysReportData("Bas_BuildVchReport","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FACCTBOOKID":{"FNumber":""},"FYEARANDPERIOD":"","FApproveBill":"false","FUnCreateVhcBill":"false","FMULACCTBOOKID":[{"FNumber":""}],"FEntity":[{"FSELECTROW":"false","FSOURCEBILL":{"FNumber":""},"FBILLFILTER":"","FBILLFILTERSTRING":"","FSubEntity":[{"FDetailID":0,"FBillID":""}]}]}}");
...
五、字段说明：
	 仅显示必须生成凭证的单据：FApproveBill 
	 仅显示未生成总账凭证的单据：FUnCreateVhcBill 
	 账簿：FACCTBOOKID  (必填项)
	 核算组织：FACCTENTITY 
	 核算体系：FAcctSystemID 
	 会计期间：FYEARANDPERIOD  (必填项)
	 账簿：FMULACCTBOOKID  (必填项)
	 选中行：FSELECTROW 
	 来源系统：FSOURCESYTEM 
	 来源单据：FSOURCEBILL 
	 单据范围：FBILLFILTER 
	 单据过滤Sql字符串：FBILLFILTERSTRING 
	 单据ID：FBillID 
...
参数FieldKeys显示列：
	 ...
	 单据创建人：FBILLCREATOR 
	 单据审核人：FBILLAUDITOR 
	 是否需要生成凭证：FNeeVoucherMsg 
	 不需生成凭证原因：FReasonForNot 
	 是否生成业务凭证：FHaveBizVch 
	 业务凭证编号：FBizVchNo 
	 是否生成总账凭证：FHaveGlVch 
	 凭证字：FGlVchGrpName 
	 凭证号：FGlVchGrpNO 
	 检查项：FCheckItem 
	 未生成凭证原因：FReasonForNoVch 
	 单据组织：FBILLORG 
