# 采购订单执行明细表_入库合并 Web API 操作原始内容

> 来源页面：全部 > 供应链 > 采购管理 > 采购订单执行明细表_入库合并

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 3208 |
...
参数FieldKeys显示列：
	 ...
	 供应商名称：FSUPPLIERNAME 
	 供应商地址：FSUPPLIERADDRESS 
	 要求交货日期：FDELIVERYDATE 
	 更新交货日期：FUPDATEDELIVERYDATE 
	 入库日期：FINSTOCKDATE 
	 入库数量：FINSTOCKQty 
	 退货日期：FBACKSTOCKDATE 
	 退货数量：FBACKSTOCKQTY 
