# 批退原因统计表 Web API 操作原始内容

> 来源页面：全部 > 共享服务中心 > 任务共享中心 > 批退原因统计表

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 2460 |
...
client.GetSysReportData("ASC_RejectResumeRpt","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FBeginDate":"1900-01-01","FEndDate":"1900-01-01","FShareCenter":[{"FNumber":""}],"FProcTemplate":[{"FNumber":""}],"FReceiver":[{"FUserAccount":""}]}}");
...
参数FieldKeys显示列：
	 ...
	 处理人：FReceiver 
	 任务总数：FFinishedQty 
	 批退总数：FRejectTotalQty 
	 批退率：FRejectTotalRate 
	 批退原因：FRejectResume 
	 批退数量：FRejectQty 
	 占比：FRejectRate 
	 模板所属组织：FOrgName 
