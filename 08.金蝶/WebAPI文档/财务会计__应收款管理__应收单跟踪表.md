# 应收单跟踪表 Web API 操作原始内容

> 来源页面：全部 > 财务会计 > 应收款管理 > 应收单跟踪表

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 3291 |
...
client.GetSysReportData("AR_TraceReport","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FReceiveNoFrom":"","FReceiveNoTo":"","FSettleOrgLst":"","FSupplierIdFrom":{"FNumber":""},"FSupplierIdTo":{"FNumber":""},"FStartDate":"1900-01-01","FEndDate":"1900-01-01","FCurrencyId":{"FNumber":""},"FContainsUnAudit":"false","FEXCLUDEB2CAR":"false"}}");
...
五、字段说明：
	 ...
	 结束日期：FEndDate 
	 币别：FCurrencyId 
	 包含未审核单据：FContainsUnAudit 
	 单据编号：FReceiveNoFrom 
	 至：FReceiveNoTo 
	 不包含B2C单据：FEXCLUDEB2CAR 

参数FieldKeys显示列：
	 单据编号：FSRECEIVEBILLNO 
	 ...
	 收款单业务日期：FRECEIVEDATE 
	 付款方：FCHARGENAME 
	 收款金额：FREAMOUNT 
	 冲销单号：FCXBILLNO 
	 应收冲销金额：FSPECIALAMOUNTFOR 
	 收款币别：FCURRENCYRECNAME 
	 收款币别金额：FREFUNDAMOUNTFOR 
	 备注：FREMARK 
	 发票号码：FIVNUMBER 
