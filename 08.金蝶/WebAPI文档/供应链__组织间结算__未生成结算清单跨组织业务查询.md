# 未生成结算清单跨组织业务查询 Web API 操作原始内容

> 来源页面：全部 > 供应链 > 组织间结算 > 未生成结算清单跨组织业务查询

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 2616 |
...
client.GetSysReportData("IOS_NotSettleIOSBillRpt","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FBillStartDate":"1900-01-01","FBillEndDate":"1900-01-01","FObjStartOrignID":{"FNUMBER":""},"FACCTSysId":{"FNUMBER":""},"FBizOrgId":"","FACCTOrgId":{"FNUMBER":""},"FIOSBizFormId":""}}");
...
五、字段说明：
	 起始日期：FBillStartDate  (必填项)
	 截止日期：FBillEndDate  (必填项)
...
参数FieldKeys显示列：
	 供货方编码：FOUTOWNERNUMBER 
	 供货方名称：FOUTOWNERNAME 
	 接收方编码：FINOWNERNUMBER 
	 接收方名称：FINOWNERNAME 
	 业务单据名称：FFORMNAME 
	 业务单据日期：FBIZDATE 
	 单据编号：FBIZBILLNO 
