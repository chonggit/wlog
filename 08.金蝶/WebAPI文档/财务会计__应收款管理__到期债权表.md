# 到期债权表 Web API 操作原始内容

> 来源页面：全部 > 财务会计 > 应收款管理 > 到期债权表

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 3560 |

## 查询报表数据 (`GetSysReportData`)

```text
一、请求参数说明：
1.formid：业务对象表单Id，字符串类型（必录）
2.data：JSON格式数据（详情参考JSON格式数据）（必录）
     2.1.FieldKeys：需查询的字段key集合，字符串类型，格式："key1,key2,..."（必录）注（简单账表可能存在动态字段，指定待查字段时，须确保当前查询条件的结果集中包含待查字段）
     2.2.SchemeId：过滤方案内码，字符串类型
     2.3.StartRow：开始行索引，整型（非必录）
     2.4.Limit：最大行数，整型，不能超过10000（非必录）
     2.5.IsVerifyBaseDataField：是否验证所有的基础资料有效性，布尔类，默认true（非必录）
     2.6.FilterString：过滤条件，数组类型，如：[{"Left":"(","FieldName":"Field1","Compare":"67","Value":"111","Right":")","Logic":"0"},{"Left":"(","FieldName":"Field2","Compare":"67","Value":"222","Right":")","Logic":"0"}]
     2.7.Model：表单数据包，JSON类型（必录）

二、返回结果：
{"Result": {"IsSuccess": true,"RowCount": 0,"Rows": [ ] }}

三、代码示例：
// 引用SDK组件Kingdee.BOS.WebApi.Client.dll; SDK下载地址：https://openapi.open.kingdee.com/ApiSdkCenter
var client = new K3CloudApi();
// 初始化登录认证，appID、appSec可在"第三方系统登录授权"中获取
client.InitClient("6871005268bf9d", "appID", "appSec", "userName", 2052, "100", "http://xherp.ipecn.com.cn/k3cloud/");
client.GetSysReportData("AR_MaturedDebt","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FAffiliation":{"FNAME":""},"FCONTACTUNITTYPE":"","FContactUnitFrom":{"FNumber":""},"FContactUnitTo":{"FNumber":""},"FSettleOrgLst":"","FIsFromFilter":"false","FDays":0,"FUNAUDIT":"false","FUNEND":"false","FCurrencyFrom":{"FNumber":""},"FByDate":"1900-01-01","FIncludePayEvaluate_New":"false","FOnlyShowPayEvaluate_New":"false","FShowLocal":"false","FReSetAllocateExc":"false","FExChangeRateType":{"FNUMBER":""}}}");

四、JSON格式数据：
{
    "FieldKeys": "",
    "SchemeId": "",
    "StartRow": 0,
    "Limit": 2000,
    "IsVerifyBaseDataField": "true",
    "FilterString": [],
    "Model": {
        "FAffiliation": {
            "FNAME": ""
        },
        "FCONTACTUNITTYPE": "",
        "FContactUnitFrom": {
            "FNumber": ""
        },
        "FContactUnitTo": {
            "FNumber": ""
        },
        "FSettleOrgLst": "",
        "FIsFromFilter": "false",
        "FDays": 0,
        "FUNAUDIT": "false",
        "FUNEND": "false",
        "FCurrencyFrom": {
            "FNumber": ""
        },
        "FByDate": "1900-01-01",
        "FIncludePayEvaluate_New": "false",
        "FOnlyShowPayEvaluate_New": "false",
        "FShowLocal": "false",
        "FReSetAllocateExc": "false",
        "FExChangeRateType": {
            "FNUMBER": ""
        }
    }
}

五、字段说明：
	 币别：FCurrencyFrom 
	 往来单位类型：FCONTACTUNITTYPE 
	 往来单位：FContactUnitFrom 
	 包括未到期单据：FUNEND 
	 日期：FByDate  (必填项)
	 包括未审核单据：FUNAUDIT 
	 提前天数：FDays 
	 来自过滤条件：FIsFromFilter 
	 至：FContactUnitTo 
	 结算组织：FSettleOrgLst  (必填项)
	 隶属方案：FAffiliation 
	 包含暂估应收：FIncludePayEvaluate_New 
	 仅显示暂估应收：FOnlyShowPayEvaluate_New 
	 显示本位币：FShowLocal 
	 本位币余额按期末汇率重算：FReSetAllocateExc 
	 调汇汇率类型：FExChangeRateType  (必填项)

参数FieldKeys显示列：
	 往来单位编码：FContactUnitNumber 
	 往来单位：FCONTACTUNIT 
	 币别：FCURRENCY 
	 收付组织：FPAYORGNAME 
	 结算组织：FSETTLEORG 
	 销售组织：FSALEORG 
	 销售部门：FSALEDEPT 
	 销售组：FSALEGROUP 
	 销售员：FSALER 
	 单据类型：FBILLTYPE 
	 单据编号：FBILLNO 
	 业务日期：FDATE 
	 到期日：FENDDATE 
	 应收金额(原币)：FBALANCEFOR 
	 币别(本位币)：FMAINCURRENCY 
	 应收金额(本位币)：FBALANCE 
	 过期天数：FMATUREDDAYS 
	 未过期天数：FUNMATUREDDAYS 
	 备注：FREMARK 
	 立账类型：FSetAccountType 
	 收款条件：FPayConditon 
	 是否转销生成：FISWRITEOFF 

备注：错误代码MsgCode说明
           0：默认
           1：上下文丢失
           2：没有权限
           3：操作标识为空
           4：异常
           5：单据标识为空
           6：数据库操作失败
           7：许可错误
           8：参数错误
           9：指定字段/值不存在
           10：未找到对应数据
           11：验证失败
           12：不可操作
           13：网控冲突
           14：调用限制
           15：禁止管理员登录
```
