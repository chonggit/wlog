# 角色字段权限报表 Web API 操作原始内容

> 来源页面：全部 > 基础管理 > 安全管理 > 角色字段权限报表

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 2096 |
...
client.GetSysReportData("SEC_RoleFieldPermissionDataRpt","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FViewPerState":"","FEditPerState":"","FForbidStatus":""}}");
...
五、字段说明：
	 查看权限状态：FViewPerState  (必填项)
	 编辑权限状态：FEditPerState  (必填项)
...
参数FieldKeys显示列：
	 ...
	 业务领域：FTopClassName 
	 子系统：FSubSystemName 
	 业务对象：FObjectTypeName 
	 字段名称：FFieldName 
	 查看：FViewPermissionStatusName 
	 编辑：FEidtPermissionStatusName 
	 禁用状态：FForbidStatusName 
