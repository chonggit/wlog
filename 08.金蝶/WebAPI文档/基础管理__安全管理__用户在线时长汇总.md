# 用户在线时长汇总 Web API 操作原始内容

> 来源页面：全部 > 基础管理 > 安全管理 > 用户在线时长汇总

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 2013 |
...
client.GetSysReportData("SEC_USERONLINEGATHER","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FStartTime":"1900-01-01","FEndTime":"1900-01-01"}}");
...
参数FieldKeys显示列：
	 用户名称：FUserName 
	 在线天数：FONLINEDAY 
	 累计在线时长（小时）：FTOTLEDURATION 
	 平均每天在线时长（小时）：FAVERAGEDURATION 
	 累计登录次数：FTOTLELOGINCNT 
	 平均每天登录次数：FAVERAGELOGINCOUNT 
	 是否禁用：FIsForbidden 
	 用户许可分组：FAppGroups 
	 用户部门分组：FPRIMARYGROUP 
