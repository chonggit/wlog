# 财务部销售收入报表 Web API 操作原始内容

> 来源页面：全部 > 供应链 > 销售管理 > 财务部销售收入报表

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 3108 |
...
client.GetSysReportData("QRDO_CWBXSSRBB","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FPurchaseOrgIdList":"","FBeginCustomerId":{"FNUMBER":""},"FEndCustomerId":{"FNUMBER":""},"FBeginMaterialId":{"FNumber":""},"FEndMaterialId":{"FNumber":""},"FCustSHORTNAME":"","FPartMonth":0,"FPartYear":0,"FALLMonth":"false","FALLMonth1":"false"}}");
...
五、字段说明：
	 销售组织：FPurchaseOrgIdList  (必填项)
	 至：FEndCustomerId 
	 客户编码：FBeginCustomerId 
	 至：FEndMaterialId 
	 物料编码：FBeginMaterialId 
	 客户所属集团：FCustSHORTNAME 
	 月份：FPartMonth 
	 年份：FPartYear 
	 显示所有月份数据：FALLMonth  (必填项)
	 显示所有月份数据：FALLMonth1  (必填项)

参数FieldKeys显示列：
	 日期：FDate 
	 单据号：FBillNo 
	 单据类型：FBILLTYPE 
	 客户国家(Country/Territory)：FCUSTCOUNTRY 
	 客户地区：FCUSTPROVINCIAL 
	 客户ID：FCUSTOMERID 
	 客户编码：FCUSTNUMBER 
	 客户名称：FCUSTNAME 
	 客户所属集团：FSHORTNAME 
	 ...
	 结算币种ID：FCURRENCYID 
	 结算币种：FCURRENCYNAME 
	 Unit Price(单价)：FPRICE 
	 金额(结算币种,不含税)：FAMOUNT 
	 汇率HKD：FEXCHANGERATE_HKD 
	 金额(HKD,不含税)：FAMOUNT_HKD 
	 美元汇率：FUSAEXCHANGERATE 
	 金额(USD,不含税)：FAMOUNT_USA 
	 组织ID：FORGID 
	 组织：FORGNAME 
	 内外销：FSALETYPE 
	 报表口径：FREPORTNAME 
