# 管易同步数据分析表 Web API 操作原始内容

> 来源页面：全部 > 供应链 > 销售管理 > 管易同步数据分析表

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 2555 |
...
client.GetSysReportData("GYSynchronousAnalyseRpt","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FOnlineShopList":"","FSoFromDate":"1900-01-01","FSoToDate":"1900-01-01","FSaleOFrom":"","FSaleOTo":"","FFormType":"","FByGYDate":"false","FMulShop":[{"FNumber":""}]}}");
...
五、字段说明：
	 网店：FOnlineShopList 
	 时间范围：FSoFromDate  (必填项)
	 至：FSoToDate  (必填项)
	 单号：FSaleOFrom 
	 至：FSaleOTo 
	 类型：FFormType  (必填项)
	 以管易单据时间为基准：FByGYDate 
	 网店范围：FMulShop  (必填项)
...
参数FieldKeys显示列：
	 ...
	 单据类型：FBILLTYPE 
	 管易云单据数：FGYBILLQTY 
	 管易云单据物料数量：FGYMATERIALSQTY 
	 管易云单据总金额：FGYBILLAMOUNT 
	 金蝶云单据数：FBILLQTY 
	 金蝶云单据物料数量：FBILLMATERIALQTY 
	 金蝶云单据总金额：FBILLAMOUNT 
	 差异单据数：FDIFFERENCEQTY 
	 差异物料数量：FDIFFEMATERIALQTY 
	 差异金额：FDIFFAMOUNT 
