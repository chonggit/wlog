# 内部利息统计表 Web API 操作原始内容

> 来源页面：全部 > 财务会计 > 资金管理 > 内部利息统计表

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 2766 |
...
client.GetSysReportData("SC_InterestStatist","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FFundsOrgID":{"FNumber":""},"FPayRecOrgID":[{"FNumber":""}],"FInterestObjID":[{"FNumber":""}],"FCurrencyID":"","FStartDate":"1900-01-01","FEndDate":"1900-01-01","FIsNotAudit":"false","FIsShowInst":"false","FIsShowPayRec":"false","FCurrencyName":""}}");
...
五、字段说明：
	 ...
	 结束日期：FEndDate  (必填项)
	 包含未审核单据：FIsNotAudit 
	 显示计息对象小计：FIsShowInst 
	 显示收付组织小计：FIsShowPayRec 
	 资金组织：FFundsOrgID  (必填项)
	 收付组织（作废）：FPayRecOrgID 
	 计息对象：FInterestObjID 
	 币别名称：FCurrencyName 
...
参数FieldKeys显示列：
	 ...
	 内部账号：FInnerAct 
	 单据编号：FBillNo 
	 计息期间：FInterestExtent 
	 存款利息：FDepositInterest 
	 透支利息：FOverAmount 
	 利息差额：FBalance 
