# 收料待检库存查询 Web API 操作原始内容

> 来源页面：全部 > 供应链 > 采购管理 > 收料待检库存查询

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 3415 |
...
client.GetSysReportData("PUR_ReceiveInsStkQueryRpt","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FPurchaseOrgIdList":"","FOrderStartDate":"1900-01-01","FOrderEndDate":"1900-01-01","FEndBillNumber":"","FBeginBillNumber":"","FEndMaterialId":{"FNUMBER":""},"FBeginMaterialId":{"FNUMBER":""},"FOwnerTypeId":"","FOwnerId":{"FNumber":""},"FDocumentStatus":"","FEntryStatus":"","FDateTypeId":"","FCheckInComing":"false"}}");
...
五、字段说明：
	 日期：FOrderStartDate 
	 至：FOrderEndDate 
	 收料组织：FPurchaseOrgIdList 
	 至：FEndBillNumber 
	 单据编号：FBeginBillNumber 
	 至：FEndMaterialId 
	 ...
	 行状态：FEntryStatus 
	 日期类型：FDateTypeId  (必填项)
	 仅来料检验：FCheckInComing 
...
参数FieldKeys显示列：
	 ...
	 收料单位：FUnitId 
	 收料数量（收料单位）：FMustQty 
	 待检数量（收料单位）：FInspCheckQty 
	 已检数量（收料数量）：FCHECKQTY 
	 待入库数量（收料单位）：FInspStockQty 
	 收料数量（库存单位）：FMustQtyStock 
	 待检数量（库存单位）：FInspCheckQtyStock 
	 已检数量（库存数量）：FCheckBaseQtyStock 
	 待入库数量（库存单位）：FInspStockQtyStock 
	 即时库存：FQueryStockQty 
	 行状态：FEntryStatus 
	 审核日期：FApproveDate 
	 来料检验：FCheckInComing 
