# 销售与库存分析报表 Web API 操作原始内容

> 来源页面：全部 > 电商与分销 > B2B电商中心 > 销售与库存分析报表

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 2911 |
...
client.GetSysReportData("CP_RPT_StockAndSale","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FSaleOrgList":"","FDateFrom":"1900-01-01","FDateTo":"1900-01-01","FCustFrom":{"FNumber":""},"FCustTo":{"FNumber":""},"FMaterialFrom":{"FNumber":""},"FMaterialTo":{"FNumber":""},"FDataSource":"","FDocumentStatus":""}}");
...
五、字段说明：
	 ...
	 产品编码：FMaterialFrom 
	 客户编码：FCustFrom 
	 销售组织：FSaleOrgList  (必填项)
	 至：FDateTo  (必填项)
	 单据状态：FDocumentStatus  (必填项)
	 至：FCustTo 
	 统计来源：FDataSource  (必填项)
...
参数FieldKeys显示列：
	 ...
	 产品编码：FMatNumber 
	 产品名称：FMaterialId 
	 销售单位编码：FSaleUnitNumber 
	 销售单位名称：FSaleUnitName 
	 基本单位编码：FBaseUnitNumber 
	 基本单位名称：FBaseUnitName 
	 规格型号：FUOM 
	 辅助属性：FAuxProp 
	 期初数量：FBeginingQty 
	 期初数量（基本）：FBaseBeginingQty 
	 订货数量：FOrderQty 
	 订货数量(基本)：FBaseOrderQty 
	 结余数量：FEndingQty 
	 结余数量(基本)：FBaseEndingQty 
	 销售数量：FSaleQty 
	 销售数量(基本)：FBaseSaleQty 
