# 银行账户余额查询 Web API 操作原始内容

> 来源页面：全部 > 财务会计 > 网上银行 > 银行账户余额查询

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 2379 |
...
client.GetSysReportData("WB_Balance","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FStartDate":"1900-01-01","FSettleOrgLst":"","FEndDate":"1900-01-01","FBankSelectNo":[{"FNumber":""}],"FCURRENCYID":{"FNumber":""},"FISBATCHBALANCE":"false"}}");
...
四、JSON格式数据：
{
    ...
    "Model": {
        ...
        "FISBATCHBALANCE": "false"
    }
}

五、字段说明：
	 查询开始日期：FStartDate  (必填项)
	 收付组织：FSettleOrgLst  (必填项)
	 银行账号：FBankSelectNo  (必填项)
	 调用批量查询：FISBATCHBALANCE 
	 币别：FCURRENCYID  (必填项)
	 查询结束日期：FEndDate  (必填项)
...
参数FieldKeys显示列：
	 ...
	 账号：FBANKNO 
	 账户名称：FNONAME 
	 昨日余额：FYESTERDAYBALANCE 
	 当前余额：FCURRENTBALANCE 
	 可用余额：FAVAILABLEBALANCE 
