# 词条统计表 Web API 操作原始内容

> 来源页面：全部 > 基础管理 > 翻译平台 > 词条统计表

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 2114 |
...
client.GetSysReportData("BOS_MLLangWordReport","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FIsContainInvalid_Filter":"false","FCATEGORY_Filter":"","FCATEGORY0_Filter":"","FDOCUMENTSTATUS_Filter":"","FDataCenterId_Filter":""}}");
...
四、JSON格式数据：
{
    ...
    "FilterString": [],
    "Model": {
        "FIsContainInvalid_Filter": "false",
        "FCATEGORY_Filter": "",
        "FCATEGORY0_Filter": "",
        "FDOCUMENTSTATUS_Filter": "",
        "FDataCenterId_Filter": ""
    }
}

五、字段说明：
	 包含失效词条：FIsContainInvalid_Filter 
	 类别：FCATEGORY_Filter 
	 词条类别：FCATEGORY0_Filter  (必填项)
	 数据状态：FDOCUMENTSTATUS_Filter  (必填项)
	 数据库列表：FDataCenterId_Filter 
