# 资产到期提示表 Web API 操作原始内容

> 来源页面：全部 > 资产管理 > 固定资产 > 资产到期提示表

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 2597 |
...
client.GetSysReportData("FA_AssetExpirationRpt","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FAssetOrgID":"","FOwnerOrgID":"","FAcctPolicyId":"","FAcctDateStart":"1900-01-01","FAcctDateEnd":"1900-01-01","FIncludeUnAudit":"false"}}");
...
四、JSON格式数据：
{
    ...
    "Model": {
        ...
        "FOwnerOrgID": "",
        "FAcctPolicyId": "",
        "FAcctDateStart": "1900-01-01",
        "FAcctDateEnd": "1900-01-01",
        "FIncludeUnAudit": "false"
    }
}

五、字段说明：
	 入账日期：FAcctDateStart  (必填项)
...
参数FieldKeys显示列：
	 ...
	 预计残值：FRESIDUALVALUE 
	 本期折旧额：FDEPR 
	 预计寿命(工作量)：FLIFEPERIODS 
	 已使用寿命（工作量）：FDEPRPERIODS 
	 剩余寿命(工作量)：FREMAINPERIODS 
	 资产编码：FASSETNO 
	 规格型号：FSPECIFICATION 
