# 内部账户账龄分析表 Web API 操作原始内容

> 来源页面：全部 > 财务会计 > 出纳管理 > 内部账户账龄分析表
...
client.GetSysReportData("CN_InnerActAnalysicReport","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FCreateOrgId":{"FNumber":""},"FMAPPINGORGIDS":[{"FNumber":""}],"FInnerAcctIds":[{"FNumber":""}],"FAffiliation":{"FNUMBER":""},"FCurrencyIds":"","FDate":"1900-01-01","FUnAudit":"false","FMyCurrency":"false","FIsCluedScInnerAcct":"false","FAgeGroupEntity":[{"FSection":"","FDays":0}]}}");
...
五、字段说明：
	 ...
	 包含未审核单据：FUnAudit 
	 显示本位币：FMyCurrency 
	 包含资金上划和下拨单的数据：FIsCluedScInnerAcct 
	 账龄分组设置：FSection 
	 天数：FDays 
...
参数FieldKeys显示列：
	 ...
	 币别：FCurrencyName 
	 对应组织：FMappingOrgName 
	 单据编号：FBillNo 
	 单据类型：FBillTypeName 
	 业务日期：FDate 
	 未还款金额：FUnRePayAmt 
	 本位币：FLocalCurrencyName 
	 未还款金额（本位币）：FUnRePayAmtFor 
