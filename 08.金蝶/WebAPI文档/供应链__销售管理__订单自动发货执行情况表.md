# 订单自动发货执行情况表 Web API 操作原始内容

> 来源页面：全部 > 供应链 > 销售管理 > 订单自动发货执行情况表

## 操作列表

| 操作 | 标识 | 文本长度 |
| --- | --- | ---: |
| 查询报表数据 | GetSysReportData | 2334 |
...
client.GetSysReportData("SAL_AutoDEExecRpt","{"FieldKeys":"","SchemeId":"","StartRow":0,"Limit":2000,"IsVerifyBaseDataField":"true","FilterString":[],"Model":{"FMulOrgId":[{"FNumber":""}],"FDateFrom":"1900-01-01","FDateTo":"1900-01-01"}}");
...
参数FieldKeys显示列：
	 ...
	 单位：FUNITID 
	 数量：FQty 
	 计划交货日期：FPLANDELIVERYDATE 
	 库存组织：FTHISSTOCKORGID 
	 仓库：FTHISSTOCKID 
	 仓位：FSTOCKLOCVALUE 
	 库存可用数量：FINVQTY 
	 本次发货数量：FTHISQTY 
